...
def test_prompt_returns_json_only() -> None:
    ...
    assert "ONLY valid JSON" in EPIC_EXTRACTION_PROMPT


def test_prompt_requests_required_attributes() -> None:
    """Verify the prompt includes all required EPIC attributes."""
    required_fields = ["title", "description", "owner", "stakeholder", "evidence"]
...
def test_prompt_contains_correct_json_shape() -> None:
    """Verify the prompt includes the correct JSON structure for EPICs."""
    assert '"items"' in EPIC_EXTRACTION_PROMPT or "'items'" in EPIC_EXTRACTION_PROMPT
